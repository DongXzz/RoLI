...
class TFDataset(torch.utils.data.Dataset):
    def __init__(self, cfg, split):
        ...
        assert cfg.DATA.NAME.startswith('vtab-cifar')
        if split == 'train':
...
def preprocess_fn(data, input_range=(0.0, 1.0)):
    image = data["image"]
...
def build_tf_dataset(cfg, mode):
    ...
    if vtab_dataname in DATASETS:
        ...
        vtab_tf_dataloader = data_cls(data_dir=data_dir, train_num_percent=cfg.DATA.TRAIN_NUM_PERCENT,
                                      train_split_percent=cfg.DATA.TRAIN_SPLIT_PERCENT)
    else:
...
def to_torch_imgs(img: np.ndarray) -> Tensor:
