...
CONFIGS = {
    "ARES_ViT_base_patch16_224_AT": configs.get_b16_config(),
    "pytorch_vitb16_224_imagenet": configs.get_b16_config(),
    "CLIP-ViT-B-16-laion2B-s34B-b88K": configs.get_clip_b16_config(),
}
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # downweight prompt
        # prompt_attn = attention_scores[:, :, :, 1:11]
        # attention_scores[:, :, :, 1:11] = prompt_attn + np.log(1e-10)
        
        # attention_scores[:, :, :, 1:11] = -1e10

        attention_probs = self.softmax(attention_scores) # B, num_head, num_patches(query), num_patches(key)

        # # manip attn
        # th = 0.9
        # prompt_attn = torch.sum(attention_probs[:, :, :, 1:11], dim=-1).flatten()
        # attention_scores_ = attention_scores.view((-1, attention_scores.shape[-1]))
        # attention_scores_[prompt_attn > th, :1] = -1e10
        # attention_scores_[prompt_attn > th, 11:] = -1e10
        # attention_scores_[prompt_attn < th, 1:11] = -1e10
        # attention_scores_ = attention_scores_.view(attention_probs.shape)
        # attention_probs = self.softmax(attention_scores_)
...
class VisionTransformer(nn.Module):
    ...
    def pytorch_load_from(self, weights, feat_dim):
        with torch.no_grad():
            ...
            self.transformer.encoder.layer.load_state_dict(weights_)
