...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 # add four parameters for lora
                 lora_rank=None, lora_droptout=None, lora_alpha=None,
                 lora_zero_init=False,
                 # add two parameters for adapter
                 adapter_type=None, adapter_reduction_factor=None
                ):
        ...
        elif adapter_reduction_factor is not None:
            self.blocks = nn.ModuleList([
                block_module(
                    dim=dim, input_resolution=input_resolution,
                    num_heads=num_heads, window_size=window_size,
                    ...
                    drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,  # noqa
                    norm_layer=norm_layer,
                    adapter_type=adapter_type, adapter_reduction_factor=adapter_reduction_factor)
                for i in range(depth)])
            self.deep_prompt = deep_prompt
