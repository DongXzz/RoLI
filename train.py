...
def setup(args):
    ...
    cfg.SOLVER.BASE_LR = cfg.SOLVER.BASE_LR/ 256 * cfg.DATA.BATCH_SIZE
    lr = cfg.SOLVER.BASE_LR
    wd = cfg.SOLVER.WEIGHT_DECAY
    pgd = cfg.ADV.TRAIN_ATTACK_STEPS if cfg.TRAIN_MODE == 'adv' else 0
    output_folder = os.path.join(
        cfg.DATA.NAME, cfg.DATA.FEATURE, 
        args.config_file.split('/')[-2], args.config_file.split('/')[-1].replace('.yaml', ''),
        f"lr{lr}_wd{wd}_pgd{pgd}")
...
def get_loaders(cfg, logger):
    logger.info(f"Loading training data {cfg.DATA.NAME}...")
